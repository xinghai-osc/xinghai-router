...
func (s *Service) listActivityLogs(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := s.db.Query(r.Context(), `
		...
		from (
			...
			select al.id::text,
				...
				al.action,al.actor,coalesce(u.name,'已删除用户'),''::text,''::text,''::text,null::integer,null::integer,null::integer,0,0,0,0,al.details,al.created_at
			from audit_logs al
	...
	for rows.Next() {
		...
		var status, duration, firstTokenMs any
		var prompt, completion, total int
		var cost, created any
		var details []byte
		if rows.Scan(&id, &logType, &action, &userID, &userName, &model, &groupID, &groupName, &status, &duration, &firstTokenMs, &prompt, &completion, &total, &cost, &details, &created) != nil {
			continue
		}
		data = append(data, map[string]any{"id": id, "type": logType, "action": action, "user_id": userID, "user_name": userName, "model": model, "group_id": groupID, "group_name": groupName, "status_code": status, "duration_ms": duration, "first_token_ms": firstTokenMs, "prompt_tokens": prompt, "completion_tokens": completion, "total_tokens": total, "cost": cost, "details": json.RawMessage(details), "created_at": created})
	}
	if rows.Err() != nil {
		writeError(w, http.StatusInternalServerError, "internal_error", "query failed")
		return
