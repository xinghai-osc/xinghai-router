...
<template>
  ...
  <main v-else class="flex min-h-screen bg-background text-foreground" :class="{ 'lg:[&>.sidebar]:w-16': sidebarCollapsed }">
    <aside class="sidebar sticky top-0 flex h-screen w-60 shrink-0 flex-col border-r border-border bg-card/50 transition-[width] lg:w-60">
        <div class="flex h-14 items-center gap-2 px-4"><span class="flex h-8 w-8 items-center justify-center rounded-lg bg-primary text-primary-foreground"><Bot :size="19" /></span><span v-show="!sidebarCollapsed" class="truncate text-sm font-semibold">{{ siteSettings.name }}</span></div>
        <nav class="flex-1 overflow-y-auto px-2 py-2">
           <div class="mb-3"><p class="px-2 pb-1 text-[10px] font-bold uppercase tracking-wider text-muted-foreground">{{ t('general') }}</p><button v-for="[id, label, Icon] in generalNav" :key="id" class="flex w-full items-center gap-2.5 rounded-md px-2 py-1.5 text-left text-sm transition-colors hover:bg-accent" :class="(id === 'usage-overview' ? (route.query.view === id || route.params.view === id) : id === 'usage' ? view === id && route.query.view !== 'usage-overview' && route.params.view !== 'usage-overview' : view === id) ? 'bg-accent font-medium text-accent-foreground' : 'text-muted-foreground'" @click="openConsole(id)"><component :is="Icon" :size="17" class="shrink-0" /><span v-show="!sidebarCollapsed" class="truncate">{{ label }}</span></button></div>
          <div class="mb-3"><p class="px-2 pb-1 text-[10px] font-bold uppercase tracking-wider text-muted-foreground">{{ t('billing') }}</p><button v-for="[id, label, Icon] in billingNav" :key="id" class="flex w-full items-center gap-2.5 rounded-md px-2 py-1.5 text-left text-sm transition-colors hover:bg-accent" :class="view === id ? 'bg-accent font-medium text-accent-foreground' : 'text-muted-foreground'" @click="openConsole(id)"><component :is="Icon" :size="17" class="shrink-0" /><span v-show="!sidebarCollapsed" class="truncate">{{ label }}</span></button></div>
          <div class="mb-3"><p class="px-2 pb-1 text-[10px] font-bold uppercase tracking-wider text-muted-foreground">{{ t('personal') }}</p><button v-for="[id, label, Icon] in personalNav" :key="id" class="flex w-full items-center gap-2.5 rounded-md px-2 py-1.5 text-left text-sm transition-colors hover:bg-accent" :class="view === id ? 'bg-accent font-medium text-accent-foreground' : 'text-muted-foreground'" @click="openConsole(id)"><component :is="Icon" :size="17" class="shrink-0" /><span v-show="!sidebarCollapsed" class="truncate">{{ label }}</span></button></div>
          <div v-if="managementNav.length" class="mb-3"><p class="px-2 pb-1 text-[10px] font-bold uppercase tracking-wider text-muted-foreground">{{ t('management') }}</p><button v-for="[id, label, Icon] in managementNav" :key="id" class="flex w-full items-center gap-2.5 rounded-md px-2 py-1.5 text-left text-sm transition-colors hover:bg-accent" :class="view === id ? 'bg-accent font-medium text-accent-foreground' : 'text-muted-foreground'" @click="openConsole(id)"><component :is="Icon" :size="17" class="shrink-0" /><span v-show="!sidebarCollapsed" class="truncate">{{ label }}</span></button></div>
        </nav>
      <div class="border-t border-border p-3"><div class="mb-2 flex items-center gap-2 text-xs text-muted-foreground"><span class="h-2 w-2 animate-pulse rounded-full bg-green-500" /><span><b class="text-foreground">{{ t('gatewayOnline') }}</b><small class="block">{{ t('serviceRunning') }}</small></span></div><div class="flex items-center gap-2"><i class="flex h-8 w-8 items-center justify-center rounded-full bg-primary text-xs font-bold text-primary-foreground">{{ account?.name?.slice(0, 1) || '?' }}</i><span v-show="!sidebarCollapsed" class="flex-1 truncate"><b class="block text-sm text-foreground">{{ account?.name || t('loadingLabel') }}</b><small class="text-xs text-muted-foreground">{{ account?.role || t('accountLabel') }}</small></span><Button v-show="!sidebarCollapsed" variant="ghost" size="icon-sm" :aria-label="t('actionAccountLogout')" :title="t('actionAccountLogout')" @click="signOut"><LogOut :size="16" /></Button></div></div>
    </aside>
